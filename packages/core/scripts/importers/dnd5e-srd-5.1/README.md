...
The committed pack holds exactly these twenty-three `table` records.
`srdGeneratedPack.test.ts` pins the table key/name set (and the per-kind
`table: 23` count) so coverage cannot silently collapse or grow without a
