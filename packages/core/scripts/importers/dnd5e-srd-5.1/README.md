...
The committed pack holds exactly these nineteen `table` records.
`srdGeneratedPack.test.ts` pins the table key/name set (and the per-kind
`table: 19` count) so coverage cannot silently collapse or grow without a
...
(eshyra-0m9.19). The `traps` anchor is
