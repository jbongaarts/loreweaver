...
The committed pack holds exactly these eighteen `table` records.
`srdGeneratedPack.test.ts` pins the table key/name set (and the per-kind
`table: 18` count) so coverage cannot silently collapse or grow without a
...
Character / Advancement table"). The money/downtime tables (Standard Exchange
...
Beyond-1st-Level reference tables (eshyra-0m9.23), the `equipment` slice for
