...
Three independent mechanisms explain why only 10 prose rules land:
...
| 16 | Traps (general guidance + sample traps + 2 tables) | 194–196 | Imported (`hazard` ×8, `table` ×2) | None (complete) | — | `loreweaver-hvp`. General trap-running prose intentionally omitted (DM procedure). |
...
- Madness → `table` ×3 + `rule` (curing) — P3
